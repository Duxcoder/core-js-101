...
function getPolynom(...args) {
  ...
  return function calc(x) {
    return result.reduce((y, item, i) => {
      if (i === 0) return y + item;
      if (i === 1) return y + (item * x);
      if (i === 2) return y + (item * x ** 2);
      return y;
    }, 0);
  };
}
...
function memoize(func) {
  ...
  return function name() {
    return memo.length ? memo[0] : memo.at(-2);
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argsToLine = args.map((arg) => JSON.stringify(arg)).join(',');
    const { name } = func;
    logFunc(`${name}(${argsToLine}) starts`);
    const result = func(...args);
    logFunc(`${name}(${argsToLine}) ends`);
    return result;
  };
}
