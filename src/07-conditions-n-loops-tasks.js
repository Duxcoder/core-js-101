...
function getSumBetweenNumbers(n1, n2) {
  if (n1 >= n2) return n1;
  const nn = n1 + 1;
  return n1 + getSumBetweenNumbers(nn, n2);
}
 ...
 *   10,10,10 =>  true
 */
function isTriangle(a, b, c, i = 0) {
  if (i === 3) return true;
  const count = i + 1;
  return a + b > c && isTriangle(b, c, a, count);
}
...
function findFirstSingleChar(str) {
  ...
  const value = Object.entries(obj).find((item) => item[1] === 0);
...
function isBracketsBalanced(str) {
  ...
  const stack = [];
  arrStr.forEach((bracket) => {
    if (open.includes(bracket)) {
      stack.push(bracket);
    } else {
      const index = open.findIndex((brack) => brack === stack.at(-1));
      if (close[index] === bracket) {
        stack.pop();
      } else {
        stack.push(bracket);
      }
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1.length; i += 1) {
    ...
    for (let j = 0; j < m1[i].length; j += 1) {
      const res = m2.reduce((sum, row, k) => sum + (m1[i][k] * m2[k][j]), 0);
      if (res) {
        m3[i][j] = res;
      }
    }
